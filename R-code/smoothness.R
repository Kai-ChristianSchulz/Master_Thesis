# Gatheral 2.1 Estimating the smoothness of the volatility process

# Trying to replicate smoothness and scaling properties of the log-volatility
# process as described in Gatherals paper by using the same Oxford-Man data set


# Function for estimating the q-th moment of log-volatility increments
# with stepsize delta i.e. an empirical estimate of:
# E(|log(signa_delta) - log(sigma_0) |^q)

# Note that for delta > 1 there are several ways to put the grid over the points,
# depending on the starting position we choose. For example,
# let data=(0,2,4,1,6,7,4,6) and delta = 2, we could choose sigma = (0,4,6,4)
# or sigma = (2,1,7,6). Here, we average over all "delta"-possible starting positions
# Note, that it is possible that we are averaging over grid with different
# lengths, if e.g. data=(1,2,3,4,5,6,7) and delta=5 we would have possible grids
# (1,6) ; (2,7) ; (3) ; (4) ; (5)
# It might/ or might not be desirable to include grids of different lengths for 
# a fixed delta (philosophy: using more data points in total vs using more 
# reliable/stable estimates due to the larger size of the grids)
# Slight deviations from the Fukasawa Paper might be due to this
...
###############################################################################
# We investigate a_q for different q (for each Index) by plotting a_q vs q and 
# then applying a linear regression
###############################################################################
# SPX
a_SPX = vector()
for (i in 1:length(q))
...
###############################################################################
# We investigate b_q for different q (for each Index)  
###############################################################################
